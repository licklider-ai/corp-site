import { HISTORICAL_REUSE } from '../data/historical-reuse';
...
- His second preprint, [Same Test, Different p](https://www.licklider.ai/research/same-test-different-p/), audits paired-t and rank-test results across R, SciPy, and Julia. [Manuscript v1.0](https://doi.org/10.5281/zenodo.22763598) and [reproducibility data and code](https://doi.org/10.5281/zenodo.22763654) are public; the preprint has not been peer reviewed.
- His latest preprint, [${HISTORICAL_REUSE.title}](https://www.licklider.ai${HISTORICAL_REUSE.href}), reports a paired historical-analysis reuse evaluation. ${HISTORICAL_REUSE.summary} ${HISTORICAL_REUSE.scope} [Preprint v1 and aggregate reproduction supplement](${HISTORICAL_REUSE.doi}); not peer reviewed. This evaluates assigned product recheck use, not the public local Record Verifier MCP package or expanded released support.
- His third preprint, [Comparing three tool-assisted AI configurations for whole-submission verification of Welch reports](https://www.licklider.ai/research/whole-submission-welch-verification/), reports a fixed-panel, 648-session comparison. [The manuscript and reproducibility package](https://doi.org/10.5281/zenodo.22784595) are on Zenodo; the preprint has not been peer reviewed or preregistered.
- The ${UPSTREAM_CONTRIBUTIONS.length} numerical and method-selection reports listed below span statsmodels, SciPy, Boost.Math, R, agricolae, and Julia/HypothesisTests.jl and were submitted under his name through issue trackers or maintainer email. The agricolae REGW report was emailed on September 10, 2026; upstream confirmation is pending. ${upstreamFixCount} have matching fixes merged upstream. The statsmodels sumsquares report, the SciPy variance-range, Mann–Whitney U method-selection, Welch ANOVA weight-sum, Welch t-test degrees-of-freedom, and Studentized-range tail reports, and the R report remain open without accepted fixes. The Julia matching fix shipped in v0.12.0 and remains in v0.12.2. SciPy PR #24840 merged on September 23 with the mparray high-precision backend, which avoids these extreme-scale failures when that backend is used; it does not repair ordinary NumPy float64 arithmetic. SciPy PR #26135 proposes a NumPy float64 variance-range repair and remains open, mergeable, and unreviewed. The R tracker could not be rechecked on September 9; its open status is the last recorded state.
